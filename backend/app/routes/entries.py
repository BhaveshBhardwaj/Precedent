...
async def create_incident(incident: IncidentCreate, background_tasks: BackgroundTasks):
    ...
    try:
        ...
        if count % 5 == 0 and count > 0:
            background_tasks.add_task(reinforce_patterns)
    except Exception:
        pass  # Non-critical
...
async def list_incidents(include_decommissioned: bool = False):
    """Get all incidents for the demo user, newest first."""
    incidents = await sqlite.get_incidents(include_decommissioned=include_decommissioned)
...
async def update_root_cause(incident_id: str, body: RootCauseUpdate, background_tasks: BackgroundTasks):
